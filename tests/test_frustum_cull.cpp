...
static void test_frustum_cull_(void) {
    ...
    Mat4x4F32 vp = app_world_camera_view_proj_(eye, target, up, 1.0472f, 1.0f, 0.1f, 100.0f);

    F32 planes[24];
    app_world_frustum_planes_(&vp, planes);
    ...
    TEST_CHECK(app_world_sphere_visible_(planes, origin, 1.0f));

    F32 behind[3] = {0.0f, 0.0f, -20.0f};
    TEST_CHECK(!app_world_sphere_visible_(planes, behind, 1.0f));

    F32 farRight[3] = {100.0f, 0.0f, 0.0f};
    TEST_CHECK(!app_world_sphere_visible_(planes, farRight, 1.0f));

    // Conservative: a sphere whose center is out but whose radius reaches
    // back across the plane stays visible.
    TEST_CHECK(app_world_sphere_visible_(planes, farRight, 200.0f));
    ...
    TEST_CHECK(!app_world_sphere_visible_(planes, beforeNear, 0.01f));
    F32 pastNear[3] = {0.0f, 0.0f, -9.5f};
    TEST_CHECK(app_world_sphere_visible_(planes, pastNear, 0.01f));

    // Far plane at view depth 100 → world z = 90.
    F32 beyondFar[3] = {0.0f, 0.0f, 95.0f};
    TEST_CHECK(!app_world_sphere_visible_(planes, beyondFar, 0.01f));
    F32 withinFar[3] = {0.0f, 0.0f, 80.0f};
    TEST_CHECK(app_world_sphere_visible_(planes, withinFar, 0.01f));
}
