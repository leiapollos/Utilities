...
// Module inputs come from build/module_inputs.txt, generated by sob at
// module build time (a compiler dep scan of the active project's TU).
// The host re-reads it when its mtime changes; no manifest means hot
// reload stays off, loudly, until './sob module' writes one.
#define HOST_MODULE_INPUTS_MANIFEST_PATH "build/module_inputs.txt"

static char g_hostModuleInputsText[96u * 1024u];
static const char* g_hostModuleInputs[2048];
static U32 g_hostModuleInputCount;
static U64 g_hostModuleInputsManifestTimestamp;
static B32 g_hostModuleInputsWarned;

static void host_refresh_module_inputs(void) {
    OS_FileInfo manifestInfo = OS_get_file_info(HOST_MODULE_INPUTS_MANIFEST_PATH);
    if (!manifestInfo.exists) {
        if (!g_hostModuleInputsWarned) {
            LOG_ERROR("host", "Module inputs manifest '{}' missing; hot reload disabled until './sob module' writes it",
                      str8(HOST_MODULE_INPUTS_MANIFEST_PATH));
            g_hostModuleInputsWarned = 1;
        }
        g_hostModuleInputCount = 0u;
        return;
    }
    if (manifestInfo.lastWriteTimestampNs == g_hostModuleInputsManifestTimestamp) {
        return;
    }
    g_hostModuleInputsManifestTimestamp = manifestInfo.lastWriteTimestampNs;
    g_hostModuleInputsWarned = 0;

    OS_Handle file = OS_file_open(HOST_MODULE_INPUTS_MANIFEST_PATH, OS_FileOpenMode_Read);
    if (!file.handle) {
        g_hostModuleInputCount = 0u;
        return;
    }
    RangeU64 range = {0ull, sizeof(g_hostModuleInputsText) - 1u};
    U64 readBytes = OS_file_read(file, range, g_hostModuleInputsText);
    OS_file_close(file);
    g_hostModuleInputsText[readBytes] = 0;

    U32 count = 0u;
    char* at = g_hostModuleInputsText;
    while (*at && count < (U32)ARRAY_COUNT(g_hostModuleInputs)) {
        while (*at == '\n' || *at == '\r') {
            at++;
        }
        if (!*at) {
    ...
    g_hostModuleInputCount = count;
    LOG_INFO("host", "Module inputs manifest loaded: {} files", count);
}

// The active project name, recorded by sob at module build time; names
// the window and the rebuild command. Default: demo.
static void host_read_module_project(char* buffer, U64 capacity) {
    ASSERT_ALWAYS(capacity >= 8u);
    MEMCPY(buffer, "demo", 5);
    OS_Handle projectFile = OS_file_open("build/module_project.txt", OS_FileOpenMode_Read);
    if (!projectFile.handle) {
        return;
    }
    RangeU64 range = {0ull, capacity - 1u};
    U64 readBytes = OS_file_read(projectFile, range, buffer);
    OS_file_close(projectFile);
    buffer[(readBytes < capacity) ? readBytes : capacity - 1u] = 0;
    for (U64 at = 0u; at < capacity; ++at) {
        if (buffer[at] == '\n' || buffer[at] == '\r') {
            buffer[at] = 0;
            break;
        }
...
static U64 host_get_newest_module_input_timestamp(void) {
    host_refresh_module_inputs();
    return host_get_newest_input_timestamp(g_hostModuleInputs,
                                          g_hostModuleInputCount,
                                          0);
}
...
static void host_try_build_module(HostState* state) {
    // The generated input list runs a few hundred files; polling that
    // many mtimes per frame is waste. 4 Hz keeps reloads instant enough.
    static U64 lastPollNanos;
    U64 nowNanos = OS_get_time_nanoseconds();
    if (lastPollNanos != 0ull && nowNanos - lastPollNanos < 250000000ull) {
        return;
    }
    ...
    char projectName[64];
    host_read_module_project(projectName, sizeof(projectName));
    Temp scratch = get_scratch(0, 0);
...
static B32 host_create_main_window(HostState* state) {
    ...
    host_read_module_project(windowTitle, sizeof(windowTitle));
