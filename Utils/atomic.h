...
namespace utils {
    ...
    public:
        ...
        UTILS_ALWAYS_INLINE void store(int x,
            ...
            const long long val = static_cast<long long>(x);
#if defined(_MSC_VER) && defined(_M_X64)
            switch (order) {
                case memory_order_relaxed:
                    _val = val;
        ...
        UTILS_ALWAYS_INLINE bool compare_exchange(int& expected, int desired,
                                                    memory_order order = memory_order_seq_cst) {
            long long old_val = static_cast<long long>(expected);
            const long long new_val = static_cast<long long>(desired);
