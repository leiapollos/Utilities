...
static void debug_draw_rect(EngContext* ctx, F32 minX, F32 minY, F32 maxX, F32 maxY, U32 rgba8) {
    draw2d_rect(&ctx->engine->render2d.draw2d, Draw2DLayer_Debug, minX, minY, maxX, maxY, rgba8);
}

static void debug_draw_box(EngContext* ctx, F32 minX, F32 minY, F32 maxX, F32 maxY, F32 thickness, U32 rgba8) {
    draw2d_box(&ctx->engine->render2d.draw2d, Draw2DLayer_Debug, minX, minY, maxX, maxY, thickness, rgba8);
}

static void debug_draw_line(EngContext* ctx, F32 x0, F32 y0, F32 x1, F32 y1, F32 thickness, U32 rgba8) {
    draw2d_line(&ctx->engine->render2d.draw2d, Draw2DLayer_Debug, x0, y0, x1, y1, thickness, rgba8);
}

static B32 eng_debug_project_point_(const ShdWorldFrameRecord* frame, F32 windowWidth, F32 windowHeight,
...
static void eng_debug_draw_record_bounds_(EngContext* ctx, const EngWorldState* world,
...
static void eng_debug_draw_world_bounds(EngContext* ctx, U32 maxBounds) {
    ...
    for (U32 lane = 0u; lane < world->laneCount && drawn < maxBounds; ++lane) {
        ...
        for (U32 renderableIndex = 0u; renderableIndex < count; ++renderableIndex) {
            eng_debug_draw_record_bounds_(ctx, world, writer->records + renderableIndex,
...
static void debug_draw_text(EngContext* ctx, EngRendererFrame* rendererFrame, F32 x, F32 y, F32 pixelSize, U32 rgba8, StringU8 text) {
    ...
    eng_renderer_submit_text(ctx, rendererFrame, &drawData, Draw2DLayer_Debug);
}
