...
static Mat4x4F32 eng_world_camera_view_proj_(Vec3F32 eye, Vec3F32 target, Vec3F32 up,
...
static void eng_world_frustum_planes_(const Mat4x4F32* m, F32* outPlanes) {
...
static B32 eng_world_sphere_visible_(const F32* planes, const F32* center, F32 radius) {
...
static F32 eng_world_transparent_depth_(const F32* center, F32 radius,
...
static void eng_world_order_ascending_(const F32* depths, U32* order, U32* scratch, U32 count) {
...
static void eng_world_builder_vertex_(EngWorldMeshBuilder* builder, F32 px, F32 py, F32 pz,
...
static void eng_world_builder_index_(EngWorldMeshBuilder* builder, U32 a, U32 b, U32 c) {
...
static void eng_world_build_cube_(EngWorldMeshBuilder* builder) {
    ...
    for (U32 face = 0u; face < 6u; ++face) {
        ...
        for (U32 corner = 0u; corner < 4u; ++corner) {
            ...
            eng_world_builder_vertex_(builder,
                                      nx * 0.5f + ux * s + vx * t,
                                      ny * 0.5f + uy * s + vy * t,
                                      nz * 0.5f + uz * s + vz * t,
                                      nx, ny, nz,
        ...
        eng_world_builder_index_(builder, base + 0u, base + 1u, base + 2u);
        eng_world_builder_index_(builder, base + 0u, base + 2u, base + 3u);
    }
}

static void eng_world_build_sphere_(EngWorldMeshBuilder* builder, U32 rings, U32 sectors) {
    U32 base = builder->vertexCount;
    for (U32 ring = 0u; ring <= rings; ++ring) {
        ...
        for (U32 sector = 0u; sector <= sectors; ++sector) {
            ...
            eng_world_builder_vertex_(builder, x * 0.5f, y * 0.5f, z * 0.5f, x, y, z, u, v);
        }
    }
    for (U32 ring = 0u; ring < rings; ++ring) {
        for (U32 sector = 0u; sector < sectors; ++sector) {
            ...
            eng_world_builder_index_(builder, a, a + 1u, b);
            eng_world_builder_index_(builder, a + 1u, b + 1u, b);
        }
    }
}

static void eng_world_build_plane_(EngWorldMeshBuilder* builder) {
    U32 base = builder->vertexCount;
    eng_world_builder_vertex_(builder, -0.5f, 0.0f, -0.5f, 0.0f, 1.0f, 0.0f, 0.0f, 0.0f);
    eng_world_builder_vertex_(builder, 0.5f, 0.0f, -0.5f, 0.0f, 1.0f, 0.0f, 4.0f, 0.0f);
    eng_world_builder_vertex_(builder, 0.5f, 0.0f, 0.5f, 0.0f, 1.0f, 0.0f, 4.0f, 4.0f);
    eng_world_builder_vertex_(builder, -0.5f, 0.0f, 0.5f, 0.0f, 1.0f, 0.0f, 0.0f, 4.0f);
    eng_world_builder_index_(builder, base + 0u, base + 2u, base + 1u);
    eng_world_builder_index_(builder, base + 0u, base + 3u, base + 2u);
}
