...
#elif defined(_WIN32) && defined(VULKAN_LOADER_STATIC_LINKED)
static BOOL CALLBACK loader_platform_thread_once_callback(PINIT_ONCE initOnce, PVOID parameter, PVOID *context) {
    (void)initOnce;
    (void)context;
...
static inline void loader_platform_thread_once_fn(INIT_ONCE *ctl, void (*func)(void)) {
    assert(func != NULL);
    assert(ctl != NULL);
    InitOnceExecuteOnce(ctl, loader_platform_thread_once_callback, &func, NULL);
}
