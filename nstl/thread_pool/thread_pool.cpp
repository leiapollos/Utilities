...
#define JOB_SYSTEM_QUEUE_SIZE (1 << 16)
...
JobSystem* job_system_init(Arena* arena, U32 workerCount) {
    ...
    js->mainQueue = js->allQueues[totalQueues - 1]; // main queue is last
    if (workerCount > 0) {
        ...
        { //Main Thread
            WorkerParams* p = (WorkerParams*)arena_push(arena, sizeof(WorkerParams), alignof(WorkerParams));
            p->js = js;
            p->index = workerCount; // worker i gets queue index i+1
        }
    }
...
void job_system_submit(JobSystem* jobSystem, Job* job) {
    ...
    if (tls_queue) {
        if (wsdq_push(tls_queue, job)) {
            return;
        }
    }
    ASSERT_DEBUG(false && "Submitting from main or external thread");
