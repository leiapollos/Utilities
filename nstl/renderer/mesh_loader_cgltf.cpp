    ...
    if (outScene->materialCount > 0) {
        ...
        for (U32 i = 0; i < outScene->materialCount; ++i) {
            ...
            if (mat->alpha_mode == cgltf_alpha_mode_blend) {
                ...
                md->alphaCutoff = 0.0f;
                LOG_DEBUG("GLTF", "Material {} uses BLEND mode", mat->name ? mat->name : "(unnamed)");
                ...
                LOG_DEBUG("GLTF", "Material {} uses MASK mode (cutoff={})", mat->name ? mat->name : "(unnamed)", md->alphaCutoff);
            } else {
                md->alphaMode = AlphaMode_Opaque;
                md->alphaCutoff = 0.0f;
                LOG_DEBUG("GLTF", "Material {} uses OPAQUE mode", mat->name ? mat->name : "(unnamed)");
            }
        }
    }
