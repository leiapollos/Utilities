...
#define DEMO_GAME_GRAVITY 30.0f
#define DEMO_GAME_JUMP_SPEED 11.0f
#define DEMO_GAME_MOVE_SPEED 14.0f
#define DEMO_GAME_GROUND_ACCEL 10.0f
#define DEMO_GAME_AIR_ACCEL 2.0f
#define DEMO_GAME_GROUND_FRICTION 8.0f
#define DEMO_GAME_PLAYER_RADIUS 1.0f
#define DEMO_GAME_GROUND_Y (-0.05f)
#define DEMO_GAME_MAX_SPEED 50.0f
#define DEMO_GAME_GROUND_NORMAL_Y 0.7f
#define DEMO_GAME_RESOLVE_MAX_ITERATIONS 4u
...
static_assert(DEMO_GAME_MAX_SPEED * ENG_SIM_TICK_DT < DEMO_GAME_PLAYER_RADIUS,
              "speed cap breaks the discrete-collision no-tunneling bound");

#define DEMO_COLLIDER_CAP 16384u
...
enum DemoColliderKind {
    DemoCollider_Box = 0u,
    DemoCollider_Sphere = 1u,
    DemoCollider_OrientedBox = 2u,
};
...
struct DemoColliderSet {
    U32 count;
    U32 dropped;
    DemoCollider colliders[DEMO_COLLIDER_CAP];
};
...
struct DemoTickStats {
    U32 contactCount;
    F32 deepestDepth;
    Vec3F32 contactPoints[DEMO_GAME_RESOLVE_MAX_ITERATIONS];
    Vec3F32 contactNormals[DEMO_GAME_RESOLVE_MAX_ITERATIONS];
};
...
static Vec3F32 demo_collider_quat_rotate_(QuatF32 q, Vec3F32 v) {
...
static Vec3F32 demo_collider_quat_unrotate_(QuatF32 q, Vec3F32 v) {
    QuatF32 conjugate;
    conjugate.x = -q.x;
    conjugate.y = -q.y;
    conjugate.z = -q.z;
    conjugate.w = q.w;
    return demo_collider_quat_rotate_(conjugate, v);
}
...
static B32 demo_collider_box_contact_local_(F32 dx, F32 dy, F32 dz, Vec3F32 halfExtents,
                                           F32 radius, DemoContact* outContact) {
...
static B32 demo_collider_sphere_contact_(const DemoCollider* collider, Vec3F32 center, F32 radius,
                                        DemoContact* outContact) {
    F32 dx = center.x - collider->center.x;
    F32 dy = center.y - collider->center.y;
    F32 dz = center.z - collider->center.z;
    if (collider->kind == DemoCollider_Box) {
        return demo_collider_box_contact_local_(dx, dy, dz, collider->halfExtents, radius, outContact);
    }
    if (collider->kind == DemoCollider_OrientedBox) {
        Vec3F32 delta;
        delta.x = dx;
        delta.y = dy;
        delta.z = dz;
        Vec3F32 local = demo_collider_quat_unrotate_(collider->orientation, delta);
        if (!demo_collider_box_contact_local_(local.x, local.y, local.z, collider->halfExtents,
                                             radius, outContact)) {
            return 0;
        }
        outContact->normal = demo_collider_quat_rotate_(collider->orientation, outContact->normal);
        return 1;
    }
...
static void demo_game_tick_(DemoPlayerState* player, const DemoActions* actions,
                           const DemoColliderSet* colliders, U64 tickIndex,
                           DemoTickStats* outStats) {
    (void)tickIndex;
    F32 dt = ENG_SIM_TICK_DT;
    ...
    if (player->grounded) {
        ...
        if (speed > 0.0f) {
            F32 drop = speed * DEMO_GAME_GROUND_FRICTION * dt;
    ...
    if (wishLength > 1e-6f) {
        wishX /= wishLength;
        wishZ /= wishLength;
        F32 accel = player->grounded ? DEMO_GAME_GROUND_ACCEL : DEMO_GAME_AIR_ACCEL;
        F32 current = player->velocity.x * wishX + player->velocity.z * wishZ;
        F32 missing = DEMO_GAME_MOVE_SPEED - current;
        if (missing > 0.0f) {
            F32 gain = MIN(accel * DEMO_GAME_MOVE_SPEED * dt, missing);
    ...
    if (actions->jump && player->grounded) {
        player->velocity.y = DEMO_GAME_JUMP_SPEED;
        player->grounded = 0;
    }

    player->velocity.y -= DEMO_GAME_GRAVITY * dt;
    ...
    F32 speedSq = player->velocity.x * player->velocity.x +
                  ...
                  player->velocity.z * player->velocity.z;
    if (speedSq > DEMO_GAME_MAX_SPEED * DEMO_GAME_MAX_SPEED) {
        F32 scale = DEMO_GAME_MAX_SPEED / SQRT_F32(speedSq);
        player->velocity.x *= scale;
    ...
    for (U32 iteration = 0u; iteration < DEMO_GAME_RESOLVE_MAX_ITERATIONS; ++iteration) {
        DemoContact deepest = {};
        for (U32 at = 0u; at < colliders->count; ++at) {
            DemoContact contact;
            if (demo_collider_sphere_contact_(colliders->colliders + at, player->position,
                                             DEMO_GAME_PLAYER_RADIUS, &contact) &&
        ...
        if (deepest.normal.y > DEMO_GAME_GROUND_NORMAL_Y) {
            contactGrounded = 1;
        }
        outStats->contactPoints[outStats->contactCount].x =
            player->position.x - deepest.normal.x * DEMO_GAME_PLAYER_RADIUS;
        outStats->contactPoints[outStats->contactCount].y =
            player->position.y - deepest.normal.y * DEMO_GAME_PLAYER_RADIUS;
        outStats->contactPoints[outStats->contactCount].z =
            player->position.z - deepest.normal.z * DEMO_GAME_PLAYER_RADIUS;
    ...
    F32 standHeight = DEMO_GAME_GROUND_Y + DEMO_GAME_PLAYER_RADIUS;
    B32 planeGrounded = 0;
...
static U64 demo_game_state_checksum_(const DemoPlayerState* player) {
    const F32 fields[6] = {
...
static Vec3F32 demo_game_render_position_(Vec3F32 previous, Vec3F32 current, F32 accumulator) {
    F32 alpha = accumulator * (1.0f / ENG_SIM_TICK_DT);
    Vec3F32 result;
